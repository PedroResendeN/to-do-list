...
function adicionar() {
    ...
    if (!input.value.trim()) {
        ...
        const conteudoCartao = `<div class="cartao">
            <span class="tarefa">${index}</span>
            <span class="botoes">
                <button class="checked" onclick="marcar(this)">
                ...
                |
                <button class="editar" onclick="editar(this)">
                ...
                |
                <button class="deletar" onclick="deletar(this)">
...
function editar(button) {
    let valorEditado = prompt('Edite sua tarefa: ').trim();
    if (!valorEditado) {
        while (!valorEditado) {
            valorEditado = prompt('Digite algo válido: ').trim();
        }
    }

    console.log(valorEditado)
    let campo = button.parentElement.parentElement;
    campo.querySelector(".tarefa").textContent = valorEditado
}

function deletar(button) {
    let campo = button.parentElement.parentElement;
    document.querySelector('.container').removeChild(campo);
}